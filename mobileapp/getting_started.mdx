...
and sending push notifications) and [Notifee](https://notifee.app/) account (a library used for displaying push notifications).
...
us. So if you want to publish your own version of the Trustlines App you'll have to modify the Bundle ID & Android package
...
The Trustlines App can be run in 3 modes - debug, staging and production. We do this by providing config files for
...
Later when your run `yarn` those files will be copied to `ios/firebase_environements` (at xCode build time the correct
`.plist` file will be copied to the ios bundle) and to `/android/app/src/**`

## Notifee config
The app comes with `notifee.config.json.dist` file. Rename this file to `notifee.config.json` and place the correct
Notifee API keys.
...
Your first need to install the cocoapods dependencies. First move to the `ios` folder and execute `pod install`
